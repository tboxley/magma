...
Worker.prototype.resource = function (resource, handler) {
  ...
  for ( var method in this.resources[resource] ) {
    if (
      method !== 'create' &&
      method !== 'read' &&
      method !== 'update' &&
      ...
      this.handleCustom(resource, method, this.resources[resource]);
    }
  }
};
...
Worker.prototype.handleCustom = function (resource, method, handler) {
  var getRequest = this.getRequest.bind(this);
  var getResponse = this.getResponse.bind(this);

  this.getExchange(resource + '.' + method);

  this.getQueue(resource + '.' + method, function (q) {
    q.bind(resource + '.' + method, '#');

    q.subscribe(function (message) {
      var req = getRequest(resource + '/' + method, message);
      var res = getResponse(resource + '/' + method, req, message);
...
Worker.prototype.getRequest = function (resource, message) {
  var messageData = JSON.parse(message.data.toString());
  var headers = messageData.headers;
